      ...
      if (input.length > 0) {
        input.forEach(data => {
          ...
          if (data.Key == 'D') { 
            ...
            let detail = [
              {
                text: ((data.InstituteName != old_ins && data.InstituteBranch != old_ins_branch ? data.InstituteName + (data.InstituteBranch == "" || data.InstituteBranch == "NULL" ? "" : " สาขา" + data.InstituteBranch) : '')).substring(0,24),
                fontSize: 7,
                style: 'setHeight',
                border: [false, false, false, false],
              },
      ...
      if (input.length > 0) {
        ...
        input.forEach(data => {
          ...
          if (data.Key == 'D') {
            ...
            let detail = [
              {
                text: (((data.CompanyOld == localStorage.lob && data.InstituteName != old_ins ) ? data.InstituteName  : (data.CompanyOld != localStorage.lob && data.CompanyName != old_ins) ? data.CompanyName : '')).substring(0,30) ,
                fontSize: 7,
                style: 'setHeight',
                border: [false, false, false, false],
              },
